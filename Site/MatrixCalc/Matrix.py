...
def MatrixsSum(first_matrix, second_matrix):
    result_matrix = [[first_matrix[i][j] + second_matrix[i][j] for j in range(0, len(first_matrix[i]))] for i in
                     range(0, len(first_matrix))]
...
def MatrixMultNumber(first_matrix, number):
    for i in range(00, len(first_matrix)):
        for j in range(0, len(first_matrix[i])):
            first_matrix[i][j] *= number
    return first_matrix
